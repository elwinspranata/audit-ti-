<x-auditor-layout>
    ...
    <div class="py-12">
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            <div class="mb-10 px-4 sm:px-0">
                ...
                <h1 class="text-3xl font-black text-gray-900 tracking-tight text-uppercase">CREATE AUDIT REPORT (PDF SYNC)</h1>
            ...
            <div class="mb-10 space-y-6">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div class="bg-white rounded-2xl p-6 shadow-xl border border-gray-200 flex flex-col justify-center items-center text-center">
                        ...
                        <div class="relative">
                            <span id="live-overall-rating" class="text-7xl font-black text-gray-900 tracking-tighter">0.0</span>
                        ...
                        <div class="mt-4 px-4 py-1 bg-gray-100 rounded-full">
                            <span id="rating-label" class="text-[10px] font-black uppercase text-gray-600">Not Assessed</span>
                        </div>
            ...
            <form action="{{ route('auditor.report.store', $assessment) }}" method="POST" class="space-y-8">
                @csrf
                ...
                <div class="section-card">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                        <div class="md:col-span-2">
                            ...
                            <input type="text" name="report_title" required class="form-input font-bold" value="{{ old('report_title', $templateData['report_title'] ?? '') }}">
                        </div>
                        <div>
                            <label class="form-label">Nama Perusahaan *</label>
                            <input type="text" name="company_name" required class="form-input" value="{{ old('company_name', $templateData['company_name'] ?? '') }}">
                        </div>
                        <div>
                            <label class="form-label">Sign-off Authority *</label>
                            <input type="text" name="sign_off_authority" required class="form-input" value="{{ old('sign_off_authority', $templateData['sign_off_authority'] ?? '') }}">
                        </div>
                        <div class="md:col-span-2">
                            <label class="form-label">Alamat / Departemen Pelaksana *</label>
                            <input type="text" name="company_address" required class="form-input" value="{{ old('company_address', $templateData['company_address'] ?? '') }}">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div>
                            ...
                            <input type="text" name="audit_director" required class="form-input" placeholder="Name" value="{{ old('audit_director') }}">
                            <input type="text" name="audit_director_phone" class="form-input mt-2 text-xs" placeholder="Phone (Optional)" value="{{ old('audit_director_phone') }}">
                        </div>
                        <div>
                            <label class="form-label">Audit Manager *</label>
                            <input type="text" name="audit_manager" required class="form-input" placeholder="Name" value="{{ old('audit_manager') }}">
                            <input type="text" name="audit_manager_phone" class="form-input mt-2 text-xs" placeholder="Phone (Optional)" value="{{ old('audit_manager_phone') }}">
                        </div>
                        <div>
                            <label class="form-label">Lead Auditor *</label>
                            <input type="text" name="lead_auditor_name" required class="form-input" value="{{ old('lead_auditor_name', Auth::user()->name) }}">
                            <input type="text" name="lead_auditor_phone" class="form-input mt-2 text-xs" placeholder="Phone (Optional)" value="{{ old('lead_auditor_phone') }}">
                        </div>
                    </div>
                </div>
                ...
                <div class="section-card">
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <textarea name="executive_summary" rows="5" required class="form-input" placeholder="Summarize overall results...">{{ old('executive_summary', $templateData['executive_summary'] ?? '') }}</textarea>
                        </div>
     <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label class="form-label">Background *</label>
                                <textarea name="background" rows="4" required class="form-input">{{ old('background', $templateData['base_background'] ?? '') }}</textarea>
                            </div>
                            <div>
                                <label class="form-label">Scope *</label>
                                <textarea name="scope" rows="4" required class="form-input">{{ old('scope', $templateData['base_scope'] ?? '') }}</textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-10 pt-8 border-t border-gray-100">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                            <div>
                                ...
                                <input type="text" name="officer_name" required class="form-input" value="{{ old('officer_name') }}">
                            </div>
                            <div>
                                <label class="form-label">Title (Jabatan) *</label>
                                <input type="text" name="officer_title" required class="form-input" value="{{ old('officer_title') }}">
                            </div>
                            <div>
                                <label class="form-label">Date of Response *</label>
                                <input type="date" name="officer_response_date" required class="form-input" value="{{ old('officer_response_date', date('Y-m-d')) }}">
                            </div>
                        </div>
                        <div>
                            <label class="form-label">Response Text *</label>
                            <textarea name="officer_response" rows="4" required class="form-input" placeholder="Management response to findings...">{{ old('officer_response') }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="section-card">
                    ...
                    <p class="text-xs text-gray-400 mb-8 italic">* Evaluation Distribution di atas akan dihitung otomatis berdasarkan rating proses di bawah ini.</p>
                    
                    <div id="it-process-focal-container" class="space-y-8"></div>
                </div>

                <div class="flex items-center justify-end gap-4 pt-6">
                    <a href="{{ route('auditor.assessments.show', $assessment) }}" class="text-sm font-bold text-gray-400 hover:text-black transition-colors">BATAL</a>
    ...
    <script>
        ...
        function addITProcessFocal(data = {}) {
            const container = document.getElementById('it-process-focal-container');
            const index = itProcessFocalCounter++;
            const html = `
                <div class="p-6 bg-gray-50 border border-gray-200 rounded-xl relative focal-item" id="it-process-focal-${index}">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <textarea name="it_process_focal_points[${index}][justification_text]" rows="3" class="form-input text-xs" placeholder="Evidence 1...&#10;Evidence 2...">${data.justification_text || ''}</textarea>
                        </div>
                        <div>
                            <label class="form-label">Indicators / Metrics (Per Baris)</label>
                            <textarea name="it_process_focal_points[${index}][indicators_text]" rows="3" class="form-input text-xs" placeholder="Metric 1...&#10;Metric 2...">${data.indicators_text || ''}</textarea>
                        </div>
                    </div>
                </div>
            `;
        ...
        // Auto-populate based on assessment items
        const prefilledData = @json($prefilledITProcess ?? []);
        if (prefilledData.length > 0) {
            prefilledData.forEach(f => {
                addITProcessFocal(f);
            });
        } else {
            addITProcessFocal();
