                    ...
                    <div class="grid grid-cols-1 gap-8 md:grid-cols-2">
                        ...
                        <div class="space-y-4">
                            <div class="flex justify-between border-b dark:border-gray-700 pb-2">
                                ...
                                <span id="transaction-code" class="font-medium">#{{ $transaction->transaction_code }}</span>
                            </div>
                            <div class="flex justify-between border-b dark:border-gray-700 pb-2">
                            ...
                            <div class="flex justify-between border-b dark:border-gray-700 pb-2">
                                ...
                                <span id="payment-status" class="px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase {{ $transaction->payment_status == 'paid' ? 'text-green-700 bg-green-100' : ($transaction->payment_status == 'pending' ? 'text-orange-700 bg-orange-100' : 'text-red-700 bg-red-100') }}">
                        ...
                        <div class="flex flex-col justify-center p-8 bg-gray-100/5 dark:bg-gray-700/20 rounded-2xl border border-gray-200 dark:border-gray-700">
                             @if(in_array($transaction->payment_status, ['pending', 'expired', 'cancelled', 'denied']))
                                <p id="pay-message" class="mb-4 text-sm text-center text-gray-600 dark:text-gray-400">
                                    {{ $transaction->payment_status == 'pending' ? 'Silakan selesaikan pembayaran melalui gerbang pembayaran aman Midtrans.' : 'Transaksi sebelumnya telah ' . $transaction->payment_status . '. Silakan klik tombol di bawah untuk mencoba lagi.' }}
                                </p>
                                <button id="pay-button" class="w-full py-3 px-4 font-bold text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition shadow-lg active:scale-95">
                                    {{ $transaction->payment_status == 'pending' ? 'Bayar Sekarang' : 'Coba Lagi' }}
                                </button>
    ...
    <script type="text/javascript">
        ...
        if(payBtn) {
            payBtn.onclick = async function(){
                ...
                payBtn.innerHTML = '<span class="inline-block animate-spin mr-2">↻</span> Memproses...';

                try {
                    // Fetch fresh token via AJAX
                    const response = await fetch('{{ route("payment.pay", $transaction->id) }}', {
                        method: 'POST',
                        headers: {
                    ...
                    // Update UI if transaction code changed
                    if (data.transaction_code) {
                        document.getElementById('transaction-code').innerText = '#' + data.transaction_code;
                    }
