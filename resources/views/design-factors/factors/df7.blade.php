...
<script>
    window.factorCalculate = function(context) {
        const { factorType, cobitMappings, importanceInputs, itemScores, itemBaselines, itemScoreHiddens, itemScoreDisplays, itemBaselineHiddens, itemBaselineDisplays, setChartData } = context;
        
        const roleImps = {
            support: parseFloat(document.querySelector('input[name="inputs[support][importance]"]')?.value) || 3,
            factory: parseFloat(document.querySelector('input[name="inputs[factory][importance]"]')?.value) || 3,
            turnaround: parseFloat(document.querySelector('input[name="inputs[turnaround][importance]"]')?.value) || 3,
            strategic: parseFloat(document.querySelector('input[name="inputs[strategic][importance]"]')?.value) || 3
        };

        const mapping = cobitMappings.DF7;
        let i = 0;
        for (const code in mapping) {
            const mapRow = mapping[code];
            const newScore = (mapRow[0] * roleImps.support) + (mapRow[1] * roleImps.factory) + (mapRow[2] * roleImps.turnaround) + (mapRow[3] * roleImps.strategic);
        ...
        const totalVal = (roleImps.support + roleImps.factory + roleImps.turnaround + roleImps.strategic) / 4;
        const totalBase = 3;
    ...
    window.factorInitCharts = function(Chart, factorType, chartLabels, chartData) {
        ...
        const barChart = new Chart(barCtx, {
            type: 'bar',
            data: { labels: chartLabels, datasets: [{ label: 'Relative Importance', data: chartData, backgroundColor: chartData.map(v => v >= 0 ? 'rgba(79, 124, 53, 0.7)' : 'rgba(192, 0, 0, 0.7)'), borderColor: chartData.map(v => v >= 0 ? 'rgba(79, 124, 53, 1)' : 'rgba(192, 0, 0, 1)'), borderWidth: 1 }] },
        ...
        const radarChart = new Chart(radarCtx, {
            type: 'radar',
            data: { labels: chartLabels, datasets: [{ label: 'Relative Importance', data: chartData.map(v => v + 100), backgroundColor: 'rgba(229, 180, 229, 0.3)', borderColor: 'rgba(229, 180, 229, 1)', borderWidth: 2, pointBackgroundColor: 'rgba(229, 180, 229, 1)', pointRadius: 2 }] },
