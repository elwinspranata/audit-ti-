...
class PaymentController extends Controller
{
    ...
    public function checkStatusViaMidtrans(Transaction $transaction)
    {
        try {
            ...
            // Jika error 404 (Transaction doesn't exist), jangan log sebagai error fatal
    ...
    // Tambahan: endpoint pay() agar AJAX frontend dapat meminta snap token terbaru
    public function pay($id)
    {
        ...
        // Jika transaksi sudah expired atau dibatalkan, kita izinkan "retry" 
        // dengan memberikan transaction_code baru (karena order_id di Midtrans harus unik)
        if (in_array($transaction->payment_status, ['expired', 'cancelled', 'denied'])) {
            $transaction->update([
                'transaction_code' => 'TRX-' . now()->format('YmdHis') . '-' . Str::upper(Str::random(6)),
                'payment_status' => 'pending',
                'snap_token' => null,
        ...
        try {
            // Generate snap token baru setiap kali dipanggil dari endpoint ini agar token selalu fresh
