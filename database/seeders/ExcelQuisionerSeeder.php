...
class ExcelQuisionerSeeder extends Seeder
{
    public function run()
    {
        $directory = base_path('kuisionercobit/Excel Kuisioner-20260224T133802Z-1-001/Excel Kuisioner');
        ...
        foreach ($files as $file) {
            ...
            try {
                ...
                foreach ($data as $sheetIndex => $rows) {
                    $levelNumber = $sheetIndex + 2; // Default mapping: Sheet 0 -> Level 2, Sheet 1 -> Level 3, etc.
    ...
    private function isQuestion($text)
    {
        ...
        return Str::endsWith($text, '?') || 
               Str::startsWith($lowerText, 'apakah') || 
               Str::startsWith($lowerText, 'bagaimana') || 
               Str::startsWith($lowerText, 'sejauh mana') ||
               Str::startsWith($lowerText, 'sejauhmana');
    }
